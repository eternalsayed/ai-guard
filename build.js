...
const TEXT_EXTS = new Set(['.js', '.html', '.sh', '.json', '.toml', '.txt', '.md']);
...
function copy(src, dst) {
  ...
  for (const entry of fs.readdirSync(src)) {
    const sp = path.join(src, entry), dp = path.join(dst, entry);
    if (fs.statSync(sp).isDirectory()) { copy(sp, dp); continue; }
    ...
    if (TEXT_EXTS.has(path.extname(entry))) {
      ...
      const changed = out.includes(HOSTED_URL) && raw.toString('utf8').includes('__HOSTED_URL__');
